...
// 		return (int)$total; 
	...
	{
		$data = $this->order->get_properties();
...
// debug($vpcURL);die;
	...
	function query_tx(){
		...
		$params=array('vpc_Merchant'=>$MID,
				'vpc_AccessCode'=>$access_code,
				'vpc_OrderInfo'=>$data['order_number'],
				'vpc_MerchTxnRef'=> $input->getString('vpc_MerchTxnRef'),
				'vpc_Version'=>'1',
				'vpc_Command'=>'queryDR',
		);
		// 		debug($params);die;
...
// 		debug($hashValidated);die;
		...
		if($hashValidated=="CORRECT" && $txnResponseCode=="0"){
			if($this->order->load($input->getInt('order_id'))){
			...
			hb_enqueue_message('Thanh toán đang bị tạm ngừng','warning');
		}else {
		...
		wp_redirect(site_url('index.php?view=message'));
		exit;
